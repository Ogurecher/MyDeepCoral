...
source_loader = get_office31_dataloader(case='source', batch_size=BATCH_SIZE[0])
target_loader = get_office31_dataloader(case='target', batch_size=BATCH_SIZE[1])
...
def train(model, optimizer, epoch, _lambda):
    ...
    for batch_idx in range(train_steps):
        ...
        result.append({
            ...
            'lambda': _lambda,
            'coral_loss': coral_loss.data, #coral_loss.data[0]
            'classification_loss': classification_loss.data, #classification_loss.data[0]
            'total_loss': sum_loss.data #sum_loss.data[0]
        })
        ...
        print('Train Epoch: {:2d} [{:2d}/{:2d}]\t'
              'Lambda: {:.4f}, Class: {:.6f}, CORAL: {:.6f}, Total_Loss: {:.6f}'.format(
                  ...
                  train_steps,
                  _lambda,
                  classification_loss.data, #classification_loss.data[0]
                  coral_loss.data, #coral_loss.data[0]
                  sum_loss.data #sum_loss.data[0]
              ))
...
if __name__ == '__main__':
    """
    Finding out dataset mean and std:
    
    print("Source mean : ", utils.get_dataset_mean_and_std('../LungCancerdetection/src/data/train')[0])
    print("Source std : ", utils.get_dataset_mean_and_std('../LungCancerdetection/src/data/train')[1])

    print("Target mean : ", utils.get_dataset_mean_and_std('../LungCancerdetection/src/data/test')[0])
    print("Target std : ", utils.get_dataset_mean_and_std('../LungCancerdetection/src/data/test')[1])
    """
    ...
    model = models.DeepCORAL(2)
    #utils.load_net(model, "checkpoint.tar")   <-- resume from checkpoint
